...
export default function StudentTopBar() {
    ...
    return (
        <header className={`${cardBg} border-b shadow-lg px-6 py-4`}>
            <div className="flex items-center justify-between">
                <div>
                    <h2 className={`text-xl font-bold ${textPrimary}`}>
                        Student Portal
                    </h2>
                    <p className={`text-xs ${textMuted}`}>Welcome back, {user?.name?.split(' ')[0] || 'Student'} 👋</p>
                </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className={`flex items-center gap-3 px-4 py-2 ${isDark ? 'bg-white/10' : 'bg-white/60'} rounded-xl`}>
                        <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-blue-500 to-cyan-600 flex items-center justify-center text-white font-bold shadow-md">
                            {user?.name?.substring(0, 2).toUpperCase() || 'ST'}
                        </div>
                        <div className="text-right hidden md:block">
                            <p className={`text-sm font-semibold ${textPrimary}`}>{user?.name}</p>
                            <p className={`text-xs ${textMuted}`}>Student</p>
                        </div>
                    </div>
